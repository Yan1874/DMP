...
import cn.util.{SchemaUtil, String2Type}
...
object Log2Parquet {
  def main(args: Array[String]): Unit = {
    ...
    //    val src: Dataset[String] = sparkSession.read.textFile(inputPath)
    val src: RDD[String] = sc.textFile(inputPath)
    import sparkSession.implicits._
    import org.apache.spark.sql._
    //分割符多个连续时添加限制
    val rowRDD = src.map(t=>t.split(",",-1)).filter(t=>t.length>=85).map(arr=>{
      Row(
        arr(0),
